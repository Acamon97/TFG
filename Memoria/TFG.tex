...
Los textos y documentos se consideran un saco de palabras, pero normalmente su contenido se encuentra en forma de una sola cadena de texto, es decir, todo el contenido del documento es un único atributo que contiene todas las palabras. Esto imposibilita cualquier tipo de operación, por lo que el primer paso que se debe realizar cuando se tratan textos es separar cada palabra individualmente. 

A este proceso en el que se divide todo el texto en unidades independientes utilizando los espacios en blanco y signos de puntuación como separadores se le llama \textit{tokenización} (\textit{tokenization}) \cite{TFG Jorge}.  

Sin embargo, ahora nos enfrentamos al problema de la alta dimensionalidad de trabajar con este tipo de datos puesto que cada una de esas palabras o \textit{tokens} que hemos obtenido del paso anterior es considerado un atributo, motivo por el cual necesitamos reducir su número. 

Una de las formas más inmediatas para ello es eliminar todas aquellas palabras que no aportan ningún tipo de información tales como pronombres, conjunciones, signos de puntuación, etc., que suelen aparecer con mucha frecuencia. Este tipo de términos se denominan \textit{palabras vacías} o \textit{stop words} en inglés. 

Una vez se ha reducido el número de atributos que procesar, es necesario normalizar todas las palabras restantes, es decir, transformarlas a su forma normal, reduciendo de nuevo espacio de términos. Hay dos métodos principales para aplicar dicha normalización: \textit{stemming} y \textit{lematización}. El primero trata de reducir cada palabra a su raiz, aunque a veces esto genera términos que no existen en el diccionario. El segundo método en cambio realiza una asignación de la forma base de cada palabra, eliminando múltiples derivaciones de la misma, como en el caso de formas verbales. 

Esto permite que cada palabra reciba el mismo tratamiento y podamos compararlas de forma equitativa. Por ejemplo, considerando el siguiente grupo de palabras: ``cantamos'', ``canta'',``cantaría'' y ``cantase'', terminaríamos con los términos: ``cant'', ``cant'',``cant'' y ``cant'' si aplicamos stemming o ``cantar'', ``cantar'',``cantar'' y ``cantar'' si aplicamos lematización. Si se busca una normalización rápida, stemming es la más rápida, pero el proceso de lematización es más preciso y conserva más el sentido de las palabras, por lo que es la más utilizada entre ambas.

Es habitual que el proceso de normalización utilice la \textit{categoría gramatical} o \textit{Part of Speech} en inglés (POS) para identificar el tipo de cada palabra basándose en un diccionario que contenga estos términos y así facilitar la extracción de su lema o raiz. 

Una vez seguidos estos dos pasos para reducir la cantidad de atributos a analizar y permitir un estudio igualitario de los mismos, hay que identificar aquellas palabras de más relevancia.

TF-IDF o que?????

En todos los sitio mencionan ese, pero parece que hay más formas de hacerlo???
