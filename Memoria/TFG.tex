...
\makeatother
    ...
    \geometry{verbose,tmargin=1.875in,bmargin=1.875in,lmargin=1.875in,rmargin=1.875in}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{extrinsecos}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{intrinsecos}
...
\begin{figure}[ht]
\adjustimage{width=\linewidth}{wordcluster}
...
\SetAlgoLined
  ...
  \SetKwRepeat{Do}{repetir}{hasta}
  \DontPrintSemicolon
  \KwIn{$D$: conjunto de datos con $n$ objetos.}
  \KwOut{Un conjunto de prototipos.}
  \Begin{
    generar un mapa de neuronas con vectores de pesos aleatorios\;
    \Do{queden muestras que procesar}{
      administrar un vector de entrada $D(t)$\;
      \ForEach{neurona $m$}{
        calcular la distancia euclídea entre el vector de entrada $D(t)$ y el vector de peso de la neurona $m$\;
        escoger la neurona con menor distancia: BMU\;
      }
      actualizar los pesos de las neuronas dentro del vecindario de BMU\;
    }
  }
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{som}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{coronavirus}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{mapa covid}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{directorio}
...
\subsection{Preparación de datos con NLP}
...
Como se aprecia, se han eliminado aquellas palabras comunes que no aportan nada al artículo como "and", o "in".
...
Comparando con el resultado mostrado previamente, podemos comprobar que también se ha realizado correctamente este proceso en que se han eliminado los plurales de las palabras y los tiempos verbales de palabras como "cats" y "caused"
...
Con este paso realizado podemos dar casi por concluido el apartado de preparación de los datos utilizando NLP, puesto que a continuación se utilizará el método de Selección de Características \textit{TF-IDF} visto en la sección \ref{subsubsec:Selección de Características}, que es una parte que tienen en común tanto el procesamiento de lenguaje natural como la clusterización
...
Como se ha mencionado a lo largo del documento, el volumen de datos con el que se trabaja en la clusterización de documentos es muy amplio y trabajar con dichos datos de forma cruda, sin tratar o minimizar tardaría exponencialmente más tiempo del que ya conlleva. De esta manera, utilizaremos una tecnia de reducción de dimensionalidad basada en la selección de caracteristicas, donde se escoge un subconjunto de los atributos originales como forma de representar al objeto. En este caso, se escogerán las palabras más relevantes del texto en función a su frecuencia inversa de aparición en el documento mediante la fórmula vista con anterioridad en la sección referente a este método en \ref{subsubsec:Selección de Características}. 
...
\bibitem{improved som algorithm applied in text clustering} Sun, A. (2010). Improved SOM Algorithm-HDSOM Applied in Text Clustering. 2010 International Conference on Multimedia Information Networking and Security. doi:10.1109/mines.2010.74

\bibitem{a survey of text mining techniques and applications} Gupta, V., \& Lehal, G. S. (2009). A Survey of Text Mining Techniques and Applications. Journal of Emerging Technologies in Web Intelligence, 1(1). doi:10.4304/jetwi.1.1.60-76

\bibitem{TFG Jorge} TFG Jorge

\bibitem{libro jorge} Weiss, S. M., Indurkhya, N., \& Zhang, T. (2015). Fundamentals of predictive text mining. Springer.

\bibitem{survey text} Aggarwal, C. C., \& Zhai, C. (2012). A Survey of Text Clustering Algorithms. Mininkg Text Data, 77-128. doi:10.1007/978-1-4614-3223-4\_4

\bibitem{lematizacion} Lematización. (2019, July 16). Retrieved September 18, 2020, from https://es.wikipedia.org/wiki/Lematizaci\%C3\%B3n

\bibitem{Genetic algorithm for text clustering based on latent semantic indexing} Song, W., \& Park, S. C. (2009). Genetic algorithm for text clustering based on latent semantic indexing. Computers \& Mathematics with Applications, 57(11-12), 1901-1907. doi:10.1016/j.camwa.2008.10.010

\bibitem{text mining use of tf idf to examine the relevance of words to documents} Qaiser, S., \& Ali, R. (2018). Text Mining: Use of TF-IDF to Examine the Relevance of Words to Documents. International Journal of Computer Applications, 181(1), 25-29. doi:10.5120/ijca2018917395

\bibitem{Document Representation and Dimension Reduction for Text Clustering} Shafiei, M., Wang, S., Zhang, R., Milios, E., Tang, B., Tougas, J., \& Spiteri, R. (2007, April). Document representation and dimension reduction for text clustering. In 2007 IEEE 23rd international conference on data engineering workshop (pp. 770-779). IEEE.

\bibitem{An Evaluation on Feature Selection for Text Clustering} Liu, T., Liu, S., Chen, Z., \& Ma, W. Y. (2003). An evaluation on feature selection for text clustering. In Proceedings of the 20th international conference on machine learning (ICML-03) (pp. 488-495).

\bibitem{Comparing and Combining Dimension Reduction Techniques for Efficient Text} Tang, B., Shepherd, M., Milios, E., \& Heywood, M. I. (2005, April). Comparing and combining dimension reduction techniques for efficient text clustering. In Proceeding of SIAM International Workshop on Feature Selection for Data Mining (pp. 17-26).

\bibitem{A review of feature selection methods with applications} Jović, A., Brkić, K., \& Bogunović, N. (2015, May). A review of feature selection methods with applications. In 2015 38th international convention on information and communication technology, electronics and microelectronics (MIPRO) (pp. 1200-1205). Ieee.

\bibitem{entropia} Dash, M.,\& Liu, H. (2000, April). Feature selection for clustering. In Pacific-Asia Conference on knowledge discovery and data mining (pp. 110-121). Springer, Berlin, Heidelberg.

\bibitem{A comparative study of tf idf lsi and multi words for text classification} Zhang, W., Yoshida, T., \& Tang, X. (2011). A comparative study of TF* IDF, LSI and multi-words for text classification. Expert Systems with Applications, 38(3), 2758-2765.

\bibitem{Document clustering tfidf approach} Bafna, P., Pramod, D.,  \& Vaidya, A. (2016, March). Document clustering: TF-IDF approach. In 2016 International Conference on Electrical, Electronics, and Optimization Techniques (ICEEOT) (pp. 61-66). IEEE.

\bibitem{som} Kohonen, T. (1990). The self-organizing map. Proceedings of the IEEE, 78(9), 1464-1480.

\bibitem{som foto} Rosli, N. R. M., \& Yahya, K. (2017). Using Non-supervised Artificial Neural Network for Determination of Anthropogenic Disturbance in a River System. Tropical Life Sciences Research, 28(2), 189.

\bibitem{em} Tatiraju, S., \& Mehta, A. (2008). Image Segmentation using k-means clustering, EM and Normalized Cuts. Department of EECS, 1, 1-7.

\bibitem{oms} OMS | Nuevo coronavirus - China. (2020, January 15). Retrieved September 21, 2020, from https://www.who.int/csr/don/12-january-2020-novel-coronavirus-china/es/

\bibitem{coronavirus imagen} Eckert, A., \& Higgins, D. (n.d.). Details - Public Health Image Library(PHIL). Retrieved September 21, 2020, from https://phil.cdc.gov/Details.aspx?pid=23311

\bibitem{mas coronavirus} Suárez, R. (2020, March 11). Atención: La OMS declara pandemia por el nuevo coronavirus. Retrieved September 21, 2020, from https://www.eltiempo.com/salud/coronavirus-ya-es-una-pandemia-declara-la-oms-471524
 
\bibitem{coronavirus mapa} COVID-19 situation update worldwide, as of 17 September 2020. (2020, September 17). Retrieved September 21, 2020, from https://www.ecdc.europa.eu/en/geographical-distribution-2019-ncov-cases 
 
\bibitem{mas mas coronavirus} Mapa de la enfermedad por coronavirus (COVID-19). (n.d.). Retrieved September 21, 2020, from https://google.com/covid19-map/?hl=en-ES

\bibitem{kaggle} AI, A. (2020, September 17). COVID-19 Open Research Dataset Challenge (CORD-19). Retrieved September 21, 2020, from https://www.kaggle.com/allen-institute-for-ai/CORD-19-research-challenge

\bibitem{R} What is R? (n.d.). Retrieved September 21, 2020, from https://www.r-project.org/about.html
