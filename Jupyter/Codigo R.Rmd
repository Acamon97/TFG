...
library(rjson); library(tidyverse); library(quanteda); library(feather); library(cld2); library(ggplot2); library(plotly); library(scales); library(cowplot); library(tau); library(stopwords); library(tm); library(textstem); library(pbapply)
...
30 segundos
```{r}
write_feather(covid_df, "D:/COVID/covid_df_feather_lema_abs.csv")
...
4 y media horas- (covid_df_feather_lema_abs.csv)
```{r}
covid_df$abstract_text <- apply(covid_df['abstract_text'], 2, function(s) lemmatize_strings(s))
```

```{r}
covid_df$body_text <- pbapply(covid_df['body_text'], 2, function(s) lemmatize_strings(s))
write_feather(covid_df, "D:/COVID/covid_df_feather_lematización.csv")
